...
import microbot.channels.matrix as matrix_module
from microbot.bus.events import OutboundMessage
from microbot.bus.queue import MessageBus
from microbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from microbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("microbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "microbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("microbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "microbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
