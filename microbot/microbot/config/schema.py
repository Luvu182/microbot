...
class WhatsAppConfig(Base):
    ...
    auto_bridge: bool = True  # Auto-start Node.js bridge process
...
class AgentDefaults(Base):
    ...
    workspace: str = "~/.microbot/workspace"
...
class WebConfig(Base):
    """Web UI / HTTP API configuration."""

    enabled: bool = True
    token: str = ""  # Auth token for web UI (empty = no auth required)
    cors_origins: list[str] = Field(default_factory=lambda: ["http://localhost:5173"])
    # REQUIRED: BaseChannel.is_allowed() checks this — "*" allows all web users
    # Token auth handles access control; allow_from just gates BaseChannel.is_allowed()
...
class Config(BaseSettings):
    """Root configuration for microbot."""
    ...
    model_config = ConfigDict(env_prefix="MICROBOT_", env_nested_delimiter="__")
